...
fn main() -> Result<()> {
    ...
    let pipe_server = NamedPipeServer::new()
        ...
        loop {
            ...
            if let Ok(event) = menu_rx.try_recv() {
                if event.id == tray.menu_item_settings {
                    ...
                    let _ = open::that("https://github.com/yourusername/gaming_optimizer/issues/new");
                ...
                Ok(Some(msg)) => {
                    match msg {
                        ...
                        GuiToTray::ActiveProfileChanged(new_active) => {
                            tracing::info!("Received ActiveProfileChanged from Settings: {:?}", new_active);
